...
export function DraftButton({ jobMatchId }: { jobMatchId: string }) {
  const router = useRouter();
  const [state, setState] = useState<"idle" | "loading" | "error">("idle");

  async function handleClick() {
    setState("loading");
    const res = await fetch(`/api/draft/${jobMatchId}`, { method: "POST" });
    if (!res.ok) {
      setState("error");
      return;
    }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        {state === "loading" ? "Drafting… (can take ~30-60s)" : "Draft this application"}
      </button>
